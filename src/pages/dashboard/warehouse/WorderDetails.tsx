...
const WorderDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSimilarOrders = async () => {
      if (!order) return;
      try {
        const response = await fetch(`/api/vendor/orders/similar?criteria=${order.criteria}`);
        const data = await response.json();
        setSimilarOrders(data);
      } catch (err) {
        console.error('Failed to fetch similar orders:', err);
      }
    };
  ...
  return (
    <div className="min-h-screen px-0 pt-4 pb-8 bg-white sm:px-0">
      <div className="ml-0 transition-all duration-300">
        ...
        <div className="mt-8">
          <h2 className="text-xl font-bold text-[#1E3B3B] mb-4">Similar Orders</h2>
          {similarOrders.length > 0 ? (
                ...
                <Card key={similarOrder.id} className="p-4 border rounded-lg shadow-sm sm:p-6">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Tag className={`text-xs sm:text-sm ${statusColors[similarOrder.status] || 'bg-[#EADCD6] text-[#1E3B3B]'}`}>
                        {getStatusIcon(similarOrder.status)}
                        {similarOrder.status}
                      </Tag>
                    </div>
                    <div className="text-sm text-[#6E8F89]">
                      <p>
                        <strong>Total:</strong> ${Number(similarOrder.total_amount || 0).toFixed(2)}
                      </p>
                      <p>
                        <strong>Date:</strong> {similarOrder.created_at ? format(new Date(similarOrder.created_at), 'MMM d, yyyy') : '-'}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <div className="flex items-center text-xs text-[#2A4D4D]">
                        <FiMapPin className="mr-1" size={14} />
                        {similarOrder.location?.city}, {similarOrder.location?.state}
                      </div>
                      <div className="flex items-center text-xs text-[#2A4D4D]">
                        <FiPackage className="mr-1" size={14} />
                        {similarOrder.items.length} item{similarOrder.items.length > 1 ? 's' : ''}
                      </div>
                    </div>
                  </div>
                </Card>
            ...
            <div className="text-center text-[#2A4D4D] text-sm py-4">
